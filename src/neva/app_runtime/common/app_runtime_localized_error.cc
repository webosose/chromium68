...
#include "neva/app_runtime/network_error_strings/grit/network_error_strings.h"
...
const AppRuntimeLocalizedErrorMap repost_error = {
    net::ERR_CACHE_MISS,
...
const AppRuntimeLocalizedErrorMap http_error_options[] = {
    {
        403, IDS_NET_ERROR_TEMPORARY_ISSUE,
        IDS_NET_ERROR_CHECK_NETWORK_CONNECTION,
        IDS_NET_ERROR_CONTACT_CONTENT_PROVIDER, IDS_NET_ERROR_TEMPORARY_ISSUE,
        // SUGGEST_NONE,
    },
};

const AppRuntimeLocalizedErrorMap dns_probe_error_options[] = {
    {
        error_page::DNS_PROBE_POSSIBLE, IDS_NET_ERROR_TEMPORARY_ISSUE,
        IDS_NET_ERROR_CHECK_NETWORK_CONNECTION,
        IDS_NET_ERROR_CONTACT_CONTENT_PROVIDER, IDS_NET_ERROR_TEMPORARY_ISSUE,
        // SUGGEST_RELOAD,
    },
...
const AppRuntimeLocalizedErrorMap* FindErrorMapInArray(
    const AppRuntimeLocalizedErrorMap* maps,
    size_t num_maps,
    int error_code) {
...
const AppRuntimeLocalizedErrorMap*
LookupErrorMap(const std::string& error_domain, int error_code, bool is_post) {
  return FindErrorMapInArray(net_error_options, arraysize(net_error_options),
                             error_code);
}
...
const char AppRuntimeLocalizedError::kHttpErrorDomain[] = "http";

void AppRuntimeLocalizedError::GetStrings(
    int error_code,
  ...
  AppRuntimeLocalizedErrorMap atsc_legay_box_options = {
      -201,
      IDS_NET_ERROR_UNABLE_TO_LOAD,
  ...
  const AppRuntimeLocalizedErrorMap* error_map =
  ...
  error_strings->SetString(
      "error_title", l10n_util::GetStringUTF16(options.title_resource_id));
  error_strings->SetString(
      "error_details", l10n_util::GetStringUTF16(options.details_resource_id));
  error_strings->SetString("error_guide", l10n_util::GetStringUTF16(
                                              options.error_guide_resource_id));
  ...
  if (net::ERR_NAME_RESOLUTION_FAILED == error_code ||
    ...
    error_strings->SetString(
        "error_info", l10n_util::GetStringFUTF16(options.error_info_resource_id,
                                                 failed_url_string));
  ...
  if (net::ERR_CERT_DATE_INVALID == error_code) {
    ...
    error_strings->SetString(
        ...
        l10n_util::GetStringUTF16(
            atsc_legay_box_options.error_guide_resource_id));
    error_strings->SetString(
...
base::string16 AppRuntimeLocalizedError::GetErrorDetails(
    const std::string& error_domain,
    int error_code,
    bool is_post) {
  const AppRuntimeLocalizedErrorMap* error_map =
  ...
  if (error_map)
    ...
    return l10n_util::GetStringUTF16(
        IDS_NET_ERROR_ERRORPAGES_HEADING_NOT_AVAILABLE);
}

bool AppRuntimeLocalizedError::HasStrings(const std::string& error_domain,
                                          int error_code) {
