...
//     6. Exit App/Settings Button are working but Network Menu not launched. =>
//     Working
...
var ButtonEnum = {SETTINGS_BUTTON: 1};
...
var onExitApp =
    function() {
  if (window._NetCast_ && history.length > 1)
    window.open('', '_self', '');
  window.close();
}

var onLaunchSetting =
    function(button_type, target) {
  if (window.errorPageController) {
    errorPageController.settingsButtonClick(target);
  }
}

var onLaunchNetworkSetting =
    function() {
  onLaunchSetting(ButtonEnum.SETTINGS_BUTTON, TargetEnum.NETWORK);
}

var onLaunchGeneralSetting =
    function() {
...
function getErrorInfo(error) {
  switch (error) {
    ...
    //  return hostname;
    case -201:
      return document.getElementById("errorinfo").innerHTML + " : " +
          getCurrentTime();
    default:
      return "";
  }
...
function createButtonOnPage(str_name, str_id, b_dir, func) {
  ...
  btn.tabIndex = 0 btn.type = "button";
...
function setButtonsOnPage(layoutCase, isRTL) {
  ...
  //<div id="Button_Left" class="button-position-left"><button
  //i18n-content="exitappbutton" id="ExitApp_Button" class="enyo-tool-decorator
  //moon-large-button-text moon-button moon-composite min-width spotlight"
  //tabindex="0" type="button" role="button">EXIT APP</button></div>
  //<div id="Button_Right" class="button-position-right"><button
  //i18n-content="networksettingbutton" id="NetworkSetting_Button"
  //class="enyo-tool-decorator moon-large-button-text moon-button moon-composite
  //min-width" tabindex="0" type="button" role="button">NETWORK
  //SETTINGS</button>
  //<button i18n-content="retry_button" id="Retry_Button"
  //class="enyo-tool-decorator moon-large-button-text moon-button moon-composite
  //min-width" tabindex="0" type="button"
  //role="button">RETRY</button></div></div>
...
setButtonStyle() {
  console.log(
      "Application Viewport: [" + appResolutionWidth + ", " +
      appResolutionHeight + "]");
...
onload() {
  ...
  setErrorPageMessages(
      err_info.error_title + "\u200e(\u200e" + errorCode + "\u200e)\u200e",
