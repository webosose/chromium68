...
const gfx::Point g_lsm_hide_location(254, 254);
// LSM defines hotspot for restoring to default webOS cursor
const gfx::Point g_lsm_restore_location(255, 255);
#endif
...
#if defined(OS_WEBOS)
    if (g_lsm_hide_location == location)
      HideLSMCursor(serial);
    else if (g_lsm_restore_location == location)
...
void WaylandCursor::HideLSMCursor(uint32_t serial) {
  wl_pointer_set_cursor(input_pointer_, serial, pointer_surface_,
                        g_lsm_hide_location.x(), g_lsm_hide_location.y());
  wl_surface_commit(pointer_surface_);
}

void WaylandCursor::RestoreLSMCursor(uint32_t serial) {
  wl_pointer_set_cursor(input_pointer_, serial, pointer_surface_,
                        g_lsm_restore_location.x(),
                        g_lsm_restore_location.y());
  wl_surface_commit(pointer_surface_);
}
#endif
