...
CodeCacheImpl::CodeCacheImpl(const base::FilePath& path, int max_size)
    : path_(path),
      max_size_(max_size),
...
void CodeCacheImpl::CreateBackend(const CompletionCallback callback) {
  ...
  disk_cache::CreateCacheBackend(net::CODE_CACHE, net::CACHE_BACKEND_DEFAULT,
                                 cache_path, max_size_, true, nullptr,
...
void CodeCacheImpl::DidBackendCreate(int result) {
  ...
  if (is_backend_cleaning_data_) {
    CompletionCallback doom_callback(
        base::Bind(&CodeCacheImpl::DidDoomAllEntries, base::Unretained(this)));
...
void CodeCacheImpl::DidEntryCreate(scoped_refptr<WriteData> write_data,
  ...
  if (result == OK) {
    ...
    int rv = write_data->entry()->WriteData(
        kMetadataIndex, 0, write_data->buf().get(), write_data->buf()->size(),
        callback, true);
