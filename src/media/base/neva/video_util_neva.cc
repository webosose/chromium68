...
bool ComputeVideoHoleDisplayRect(const gfx::Rect& video_rect_in_view_space,
  ...
  // Step2: Compute video_rect. This variable is essential to compute
  //        source_rect and visible_rect.
  gfx::Rect video_rect = ScaleToEnclosingRectSafe(
      video_rect_in_view_space, additional_scale.x(), additional_scale.y());

  // Step3: Compute source_rect.
  gfx::Rect view_rect_from_origin(0, 0, view_rect.width(), view_rect.height());
  visible_rect = gfx::IntersectRects(video_rect, view_rect_from_origin);
  ...
  if (visible_rect == video_rect || visible_rect.IsEmpty()) {
    source_rect = natural_video_rect;
  } else {
    DCHECK(video_rect.width() != 0 && video_rect.height() != 0);

    int source_x = visible_rect.x() - video_rect.x();
    int source_y = visible_rect.y() - video_rect.y();
    ...
    float scale_width =
        static_cast<float>(natural_video_rect.width()) / video_rect.width();
    float scale_height =
        static_cast<float>(natural_video_rect.height()) / video_rect.height();

    source_rect = gfx::Rect(source_x, source_y, visible_rect.width(),
                            visible_rect.height());
    source_rect =
        ScaleToEnclosingRectSafe(source_rect, scale_width, scale_height);
    source_rect = ScaleToEnclosingRectSafe(source_rect, additional_scale.x(),
                                           additional_scale.y());
  }
