...
VideoRendererWebOS::VideoRendererWebOS(
...
void VideoRendererWebOS::Flush(const base::Closure& callback) {
  DVLOG(1) << __func__;
  DCHECK(task_runner_->BelongsToCurrentThread());
  ...
  if (buffering_state_ != BUFFERING_HAVE_NOTHING) {
    ...
    task_runner_->PostTask(
        FROM_HERE, base::Bind(&VideoRendererWebOS::OnBufferingStateChange,
  ...
  video_frame_stream_->Reset(
      base::Bind(&VideoRendererWebOS::OnVideoFrameStreamResetDone,
...
void VideoRendererWebOS::StartPlayingFrom(base::TimeDelta timestamp) {
...
void VideoRendererWebOS::Initialize(
  ...
  video_frame_stream_->set_config_change_observer(base::Bind(
      &VideoRendererWebOS::OnConfigChange, weak_factory_.GetWeakPtr()));
  ...
  video_frame_stream_->Initialize(
      stream, base::Bind(&VideoRendererWebOS::OnVideoFrameStreamInitialized,
                         weak_factory_.GetWeakPtr()),
      cdm_context, base::Bind(&VideoRendererWebOS::OnStatisticsUpdate,
                              weak_factory_.GetWeakPtr()),
...
scoped_refptr<VideoFrame> VideoRendererWebOS::Render(
  ...
  if (buffering_state_ == BUFFERING_HAVE_ENOUGH && !received_end_of_stream_ &&
    ...
    task_runner_->PostTask(
        FROM_HERE, base::Bind(&VideoRendererWebOS::TransitionToHaveNothing,
                              weak_factory_.GetWeakPtr()));
  }
  ...
  task_runner_->PostTask(
      FROM_HERE,
      base::Bind(&VideoRendererWebOS::AttemptReadAndCheckForMetadataChanges,
...
void VideoRendererWebOS::OnFrameDropped() {
  base::AutoLock auto_lock(lock_);
  algorithm_->OnLastFrameDropped();
}

void VideoRendererWebOS::OnVideoFrameStreamInitialized(bool success) {
  DCHECK(task_runner_->BelongsToCurrentThread());
  base::AutoLock auto_lock(lock_);
...
void VideoRendererWebOS::OnPlaybackError(PipelineStatus error) {
  DCHECK(task_runner_->BelongsToCurrentThread());
  client_->OnError(error);
}

void VideoRendererWebOS::OnPlaybackEnded() {
  DCHECK(task_runner_->BelongsToCurrentThread());
  client_->OnEnded();
}

void VideoRendererWebOS::OnStatisticsUpdate(const PipelineStatistics& stats) {
  DCHECK(task_runner_->BelongsToCurrentThread());
  client_->OnStatisticsUpdate(stats);
}

void VideoRendererWebOS::OnBufferingStateChange(BufferingState state) {
  DCHECK(task_runner_->BelongsToCurrentThread());
...
void VideoRendererWebOS::SetTickClockForTesting(
    std::unique_ptr<base::TickClock> tick_clock) {
  tick_clock_.swap(tick_clock);
}

void VideoRendererWebOS::SetGpuMemoryBufferVideoForTesting(
    std::unique_ptr<GpuMemoryBufferVideoFramePool> gpu_memory_buffer_pool) {
  gpu_memory_buffer_pool_.swap(gpu_memory_buffer_pool);
}

void VideoRendererWebOS::OnTimeProgressing() {
  DCHECK(task_runner_->BelongsToCurrentThread());
...
void VideoRendererWebOS::OnTimeStopped() {
  DCHECK(task_runner_->BelongsToCurrentThread());
  ...
  if (buffering_state_ == BUFFERING_HAVE_NOTHING) {
    ...
    // TODO(neva): Consider following upstream codes.
...
void VideoRendererWebOS::FrameReady(VideoFrameStream::Status status,
  ...
  if (status == VideoFrameStream::DECODE_ERROR) {
    ...
    task_runner_->PostTask(
        FROM_HERE,
        base::Bind(&VideoRendererWebOS::OnPlaybackError,
...
bool VideoRendererWebOS::HaveEnoughData_Locked() {
  DCHECK_EQ(state_, kPlaying);
  lock_.AssertAcquired();

  if (received_end_of_stream_ && media_platform_api_->IsEOSReceived())
    return true;

  if (HaveReachedBufferingCap() && !media_platform_api_->AllowedFeedVideo())
    return true;
...
void VideoRendererWebOS::TransitionToHaveEnough_Locked() {
  DVLOG(3) << __func__;
  DCHECK(task_runner_->BelongsToCurrentThread());
  DCHECK_EQ(buffering_state_, BUFFERING_HAVE_NOTHING);
  lock_.AssertAcquired();
...
void VideoRendererWebOS::TransitionToHaveNothing() {
  DVLOG(3) << __func__;
  DCHECK(task_runner_->BelongsToCurrentThread());
...
void VideoRendererWebOS::TransitionToHaveNothing_Locked() {
  DVLOG(3) << __func__;
  DCHECK(task_runner_->BelongsToCurrentThread());
  lock_.AssertAcquired();
...
void VideoRendererWebOS::AddReadyFrame_Locked(
    const scoped_refptr<VideoFrame>& frame) {
  DCHECK(task_runner_->BelongsToCurrentThread());
  lock_.AssertAcquired();
...
void VideoRendererWebOS::AttemptRead_Locked() {
  DCHECK(task_runner_->BelongsToCurrentThread());
  lock_.AssertAcquired();

  if (pending_read_ || (received_end_of_stream_ && media_platform_api_->IsEOSReceived()))
    return;
  ...
  switch (state_) {
    case kPlaying:
      ...
      video_frame_stream_->Read(
          base::BindRepeating(&VideoRendererWebOS::FrameReady,
...
void VideoRendererWebOS::OnVideoFrameStreamResetDone() {
...
void VideoRendererWebOS::UpdateStats_Locked() {
  lock_.AssertAcquired();
  ...
  if (frames_decoded_ || frames_dropped_) {
    ...
    task_runner_->PostTask(FROM_HERE,
                           base::Bind(&VideoRendererWebOS::OnStatisticsUpdate,
...
bool VideoRendererWebOS::HaveReachedBufferingCap() {
  DCHECK(task_runner_->BelongsToCurrentThread());
...
void VideoRendererWebOS::StartSink() {
  DCHECK(task_runner_->BelongsToCurrentThread());
...
void VideoRendererWebOS::StopSink() {
  DCHECK(task_runner_->BelongsToCurrentThread());
  sink_->Stop();
...
void VideoRendererWebOS::MaybeFireEndedCallback_Locked(bool time_progressing) {
  lock_.AssertAcquired();
  ...
  if (!media_platform_api_->IsEOSReceived())
    return;
  ...
  if (!algorithm_->effective_frames_queued() ||
    ...
    task_runner_->PostTask(FROM_HERE,
                           base::Bind(&VideoRendererWebOS::OnPlaybackEnded,
                                      weak_factory_.GetWeakPtr()));
  }
}

base::TimeTicks VideoRendererWebOS::ConvertMediaTimestamp(
...
base::TimeTicks VideoRendererWebOS::GetCurrentMediaTimeAsWallClockTime() {
  std::vector<base::TimeTicks> current_time;
  wall_clock_time_cb_.Run(std::vector<base::TimeDelta>(), &current_time);
  return current_time[0];
}

bool VideoRendererWebOS::IsBeforeStartTime(base::TimeDelta timestamp) {
  return timestamp + video_frame_stream_->AverageDuration() < start_timestamp_;
}

void VideoRendererWebOS::RemoveFramesForUnderflowOrBackgroundRendering() {
...
void VideoRendererWebOS::CheckForMetadataChanges(VideoPixelFormat pixel_format,
...
void VideoRendererWebOS::AttemptReadAndCheckForMetadataChanges(
